...
public class AddContactsAction extends HttpServlet {
	/**
	 * 
	 */
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		notesDTO.setUserid((Integer) (session.getAttribute("userid")));
		try {
			BeanUtils.populate(notesDTO, map);
			flag = new NotesDelegate().addNewContacts(notesDTO);
			if (flag) {
				System.out.println("hai");
				path = UtilConstants._STATUS;
				request.setAttribute(UtilConstants._STATUS_STRING,
						UtilConstants._CONTACT_ADDED_SUCCESS);
			} else {
				path = UtilConstants._STATUS;
				request.setAttribute(UtilConstants._STATUS_STRING,
						UtilConstants._CONTACT_ADDED_FAIL);
			}
